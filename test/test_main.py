...
import pytest
from src.main import Category, Product, Smartphone, LawnGrass, Order, BaseProduct, ZeroQuantityError, add_product_with_validation
...
@pytest.fixture
def test_category_empty() -> Category:
    return Category("Пустая категория", "Без товаров")


@pytest.fixture
def test_order(test_product) -> Order:
    return Order("Заказ 1", "Первый заказ", test_product, 2)


def test_base_product_inheritance():
...
def test_category_average_price(test_category, test_category_empty):
    assert test_category.average_price() == 123000.0
    assert test_category_empty.average_price() == 0


def test_add_product_with_validation(capsys, test_category, test_product):
    add_product_with_validation(test_category, test_product)
    captured = capsys.readouterr()
    assert "Товар успешно добавлен" in captured.out
    assert "Обработка добавления товара завершена" in captured.out

    zero_product = Product("Zero Product", "Zero", 1000.0, 0)
    add_product_with_validation(test_category, zero_product)
    captured = capsys.readouterr()
...
def test_order_init(test_order: Order, test_product: Product) -> None:
    assert test_order.name == "Заказ 1"
    assert test_order.description == "Первый заказ"
    assert test_order.product == test_product
    assert test_order.quantity == 2
    assert test_order.total_price == test_product.price * 2


def test_order_str(test_order: Order) -> None:
    assert str(test_order) == f"Заказ: Заказ 1, Товар: Samsung Galaxy S23 Ultra, Количество: 2, Итоговая стоимость: {180000.0 * 2} руб."
